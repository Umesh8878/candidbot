package com.candidbot.controller;
...
import com.candidbot.model.InterviewGptResponse;
import com.candidbot.service.TextToSpeech;
...
public class InterviewAutomationController {
	...
	private void setInterviewProfile(List<String> interviewConfigModesList) {	
		...
		String profile1 =  interviewConfigModesList.stream()
				                                  .filter(param -> param.startsWith("profile"))
				                                  .findFirst().get();
		
		profile1 = profile1.replaceAll("\\s", "");
		profile1 = profile1.split("=")[1];
	...
	private void initialize() {	
		...
		interviewMockQnsList = new ArrayList<String>();
	...
	private void conductInterview() throws InterruptedException {
    	...
    	while(qsItr.hasNext() && (numQnsAskedAlready < limitNumQnsToAskFinal)){
	...
	private List<String> randomize(List<String> interviewQnsList) {
		...
		for (int i = 0; i < numOfQns; i++) {
			RandomUtils.nextInt(i+1, numOfQns+1);
	...
	private String preProcessFile(String fileName) {   // construct full file name with profile included
